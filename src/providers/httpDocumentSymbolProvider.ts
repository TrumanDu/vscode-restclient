...
export class HttpDocumentSymbolProvider implements DocumentSymbolProvider {
  public async provideDocumentSymbols(
    ...
    const allLines: string[] = document
      .getText()
    ...
    for (let i = 0; i < allSharpRowIndexs.length; i++) {
      ...
      let child1Count = this.GetChildRows(
        allLines,
        allSharpRowIndexs,
        sharpCount1,
        i
      ...
      for (let j = 0; j < child1Count; j++) {
        ...
        let child2Count = this.GetChildRows(
          allLines,
          allSharpRowIndexs,
          sharpCount11,
          i
        ...
        for (let k = 0; k < child2Count; k++) {
          ...
          let child3Count = this.GetChildRows(
            allLines,
            allSharpRowIndexs,
            sharpCount111,
            i
  ...
  private GetChildRows(
    allLines: string[],
    allSharpRowIndexs: number[],
    parentSharpCount: number,
    i: number
  ): number {
